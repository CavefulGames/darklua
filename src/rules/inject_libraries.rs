...
use crate::nodes::{Arguments, Block, FunctionCall, LocalAssignStatement, Prefix, StringExpression, TypedIdentifier};
use crate::rules::{
...
fn get_require_path(libs_path: &PathBuf, path: &PathBuf, context: &Context) -> PathBuf {
    let lib_file_stem = path
        .file_stem()
        .map(|x| {
            OsString::from(x).into_string().unwrap()
        }
    ).unwrap();
    ...
    fs::copy(path.as_path(), lib_path.as_path()).unwrap();
    let base_path = context.path.as_path().parent().expect("Could not find parent path of the source");
    let mut relative_path = diff_paths(lib_path.as_path(), base_path).expect("Could not resolve a path");
...
impl Rule for InjectLibraries {
    fn process(&self, block: &mut Block, context: &Context) -> RuleProcessResult {
        let project_path = context.project_location.as_ref().expect("Project path is required");
        let libs_path = project_path.join(self.path.as_path());
        fs::create_dir_all(&libs_path).unwrap();
        match self.require_mode.to_owned() {
            RequireMode::Path(_) => {
                for lib in &self.libraries {
                    let string_exp = StringExpression::from_value(get_require_path(&libs_path, &lib.path, context).to_slash_lossy());
                    let require_arg = Arguments::String(string_exp);

                    let require_call = FunctionCall::new(Prefix::from_name("require"), require_arg, None);
            ...
            RequireMode::Roblox(mut require_mode) => {
                ...
                for lib in &self.libraries {
                    ...
                    if let Some(require_arg) = require_mode.generate_require(
